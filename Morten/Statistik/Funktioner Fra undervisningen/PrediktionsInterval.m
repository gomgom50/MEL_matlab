function Output = PrediktionsIntervalNormal(data, procentKI, std, middel)
...
if  ~isempty(std)
    ...
    disp("Dernæst regnes " + procentKI  + "% konfidensintervallets værdier")

    displayFormula("Upper = y_bar + z_alpha/2 * sigma/sqrt(n)")

    upper_t = y_bar_x + z_alpha_halve*std*sqrt(1+1/obs);

    displayFormula("Lower = y_bar - z_alpha/2 * sigma/sqrt(n)")
    lower_t = y_bar_x - z_alpha_halve*std*sqrt(1+1/obs);
...
elseif isempty(std)
    ...
    upper_t = y_bar_x + t_alpha2*s_val*sqrt(1+1/obs);

    disp("Nedre konfidensinterval værdi")
    displayFormula("y_bar - t_df_alpha/2 * s/sqrt(n)")
    lower_t = y_bar_x - t_alpha2*s_val*sqrt(1+1/obs);
...
xs = ki_mu(1)*0.95:0.0001:ki_mu(2)*1.05;
...
area(xs(1:length(lower)), pd(1:length(lower)), "DisplayName","Nedre PI "+round(ki_mu(1),3))
area(xs(upper(1):end), pd(upper(1):end), "DisplayName","Øvre PI "+round(ki_mu(2),3))

hold off

alpha(0.5)
title(procentKI + "% Prædiktionsinterval for middelværdien")
xlabel("Middelværdi")
ylabel("% sandsynlighed")
legend('show', 'location','best')

Output.KI = ki_mu;
Output.test_alpha = test_vals;
